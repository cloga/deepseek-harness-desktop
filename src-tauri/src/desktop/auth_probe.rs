...
//! 属性的桥接存在平台差异。本脚本只在字面 IPv4 子 frame 中执行一次真实受保护
...
pub(crate) const AUTH_TOP_LEVEL_PROBE_JS: &str = r#"(function () {
  ...
  fetch('/api/settings/describe', {
    ...
    location.href = 'dsh-auth-probe://127.0.0.1/?status=' + response.status;
  }).catch(function () {
    location.href = 'dsh-auth-probe://127.0.0.1/?status=0';
  });
})();"#;
