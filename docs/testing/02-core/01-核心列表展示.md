...
[测试步骤] 1. 由 Desktop 启动核心。2. 等待核心输出认证 URL。3. 检查 Desktop 健康状态与 iframe。4. 检查服务日志与运行日志。5. 停止监听端口但暂不结束 Node 进程，再执行健康检查。
[预期结果] 1. Desktop 捕获一次性 URL 后，通过真实 HTTP 请求确认端口仍可达，并进入 ready，不会永久停在「Waiting for Harness」。2. iframe 首次使用认证 URL，服务以 303 写入 HttpOnly cookie 后正常展示；界面中的普通服务地址不含 token。3. 日志中的 token 被替换为 `<redacted>`。4. 端口停止监听后健康检查失败，不会仅凭缓存 URL 误报健康。5. 不输出认证 URL 的旧核心仍按插件 bundle 探测流程正常启动。
